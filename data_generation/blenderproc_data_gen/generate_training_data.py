...
def get_camera_fov():
    """
    Compute the horizontal and vertical FOV from the camera intrinsics.
    """
    K = bp.camera.get_intrinsics_as_K_matrix()  # Get the intrinsic matrix

    f_x, f_y = K[0, 0], K[1, 1]  # Extract focal lengths
    c_x, c_y = K[0, 2], K[1, 2]  # Extract principal points

    fov_x = 2 * np.arctan(c_x / f_x)  # Horizontal FOV
    fov_y = 2 * np.arctan(c_y / f_y)  # Vertical FOV

    return fov_x, fov_y

def place_object_in_camera_fov(camera_pose, near=10, far=80):
    """
    Places an object in the camera's field of view, ensuring it remains visible.
    """
    camera_forward = -camera_pose[:3, 2]  # Camera looks along -Z axis
    depth = random.uniform(near, far)  # Random depth

    # Choose a random offset within a certain range
    fov_x, fov_y = get_camera_fov()
    angle_x = random.uniform(-fov_x / 2, fov_x / 2)
    angle_y = random.uniform(-fov_y / 2, fov_y / 2)

    # Compute position relative to the camera
    x_offset = depth * np.tan(angle_x)
    y_offset = depth * np.tan(angle_y)

    # Convert from camera space to world space
    object_position = (camera_pose[:3, 3] + depth * camera_forward
                       + x_offset * camera_pose[:3, 0]  # X direction
                       + y_offset * camera_pose[:3, 1]) # Y direction
...
def random_rotation_matrix_object(max_tilt=np.radians(10)):
    yaw = random.uniform(0, 2 * np.pi)  # Full random yaw
    pitch = random.uniform(-max_tilt, max_tilt)  # Small pitch variation
    roll = random.uniform(-max_tilt, max_tilt)  # Small roll variation

    return Rx(np.pi/2) @ Rz(roll) @ Ry(yaw) @ Rx(pitch)
...
def get_cuboid_image_space(mesh, camera, img_width):
    ...
    world_to_cam = np.linalg.inv(camera.get_camera_pose()) # 4x4 world to camera transformation matrx.
    # camera.get_camera_pose(): camera's position and orientation within the world coordinate system
    R = world_to_cam[:3, :3].copy()
    
    rvec = cv2.Rodrigues(R)[0]
    tvec = world_to_cam[0:3, 3]
    ...
    for ii in range(8):
        ...
        cuboid[dope_order[ii]][0] = img_width - 1 - cuboid[dope_order[ii]][0]  # Flip X axis
    cuboid[8] = cv2.projectPoints(centroid, rvec, tvec, K, np.array([]))[0][0][0]
    cuboid[8][0] = img_width - 1 - cuboid[8][0]  # Flip X axis
...
def write_json(outf, args, camera, objects, objects_data, seg_map):
    ...
    for ii, oo in enumerate(objects):
        ...
        if num_pixels < args.min_pixels:
            continue
        projected_keypoints = get_cuboid_image_space(oo, camera, args.width)
...
def draw_cuboid_markers(objects, camera, im, img_width):
    colors = ['yellow', 'magenta', 'blue', 'red', 'green', 'orange', 'brown', 'cyan', 'white']
    R = 2 # radius
    # draw dots on image to label the cuiboid vertices
    draw = ImageDraw.Draw(im)
    for oo in objects:
        projected_keypoints = get_cuboid_image_space(oo, camera, img_width)
...
def main(args):
    def orthogonalize_rotation_matrix(R):
        """Fix small numerical errors in a rotation matrix via SVD"""
    ...
    random_yaw = random.uniform(0, 2*np.pi)  # Random rotation around Z-axis
    random_pitch = random.uniform(-np.pi/6, -np.pi/36)  # Small tilt down/up
    random_roll = np.clip(random.gauss(0, np.pi/36), -np.pi/12, np.pi/12)  # Small roll
    cam_pose = bp.math.build_transformation_mat(
        [0, 0, -20],  # Camera position remains the same: orginal [0, -25, 0]
        [np.pi / 2 + random_pitch, random_roll, random_yaw],  # Random rotation
    )
    ...
    for frame in range(args.nb_frames):
        ...
        for idx, oo in enumerate(objects):
            ...
            camera_pose = bp.camera.get_camera_pose()
            xform[0:3,3] = place_object_in_camera_fov(camera_pose)
            xform[0:3,0:3] = random_rotation_matrix_object()
            oo.set_local2world_mat(xform)

            # xform[0:3, 3] = [0, 0, 0]
            # xform[0:3, 0:3] = np.eye(3)
            # oo.set_local2world_mat(xform)

            # 'location' and 'quaternion_xyzw' describe the position and orientation of the
            # object in the camera coordinate system
            world_to_camera = np.linalg.inv(bp.camera.get_camera_pose())
            xform_in_cam = world_to_camera @ xform
            objects_data[idx]['location'] = xform_in_cam[0:3,3].tolist()

            R_cam = orthogonalize_rotation_matrix(xform_in_cam[0:3, 0:3])
            tmp_wxyz = Quaternion(matrix=R_cam).elements  # [scalar, x, y, z]
        ...
        for dd in distractors:
            ...
            xform[0:3, 3] = place_object_in_camera_fov(bp.camera.get_camera_pose(), near=30, far=70)
        ...
        if args.debug:
            im = draw_cuboid_markers(objects, bp.camera, im, args.width)
...
if __name__ == "__main__":
    ...
    print(f"Generating training data: {opt.run_id + 1}th set.")
    main(opt)
