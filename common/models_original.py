...
class DopeNetwork(nn.Module):
    def __init__(
        ...
        # vgg_full = models.vgg19(pretrained=False).features # original code
        if pretrained:
            print("Training from the input model, not using pretrained VGG weights.")
            # vgg_full = models.vgg19(weights=None).features
            vgg_full = models.vgg19(weights='IMAGENET1K_V1').features

            # freeze_until = 10  # freeze layers 0 through 9 for fine tuning
            # for idx, layer in enumerate(vgg_full):
            #     if idx < freeze_until:
            #         for param in layer.parameters():
            #             param.requires_grad = False

        else:
            print("Training from scratch, using pretrained VGG weights.")
            vgg_full = models.vgg19(weights='IMAGENET1K_V1').features # Use pretrained VGG weights as backbone for feature extraction
