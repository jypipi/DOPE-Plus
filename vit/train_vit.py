...
def get_cuboid_keypoints_3d(object_class="Cookies"):
    ...
    """
    cookies_dims = (0.1672, 0.0402, 0.1227)
    w, h, l = cookies_dims
    x, y, z = l / 2, w / 2, h / 2
    corners = np.array([
        [-x, -y, -z],
...
def evaluate_model(epoch, val_loader):
    ...
    for batch in val_loader:
        ...
        for i in range(data.shape[0]):
            ...
            try:
                quat_gt, t_gt, K = load_pose_and_intrinsics(gt_json_path, object_class="Cookies")
                R_gt = quat_to_matrix(quat_gt)

                object_points = get_cuboid_keypoints_3d(object_class="Cookies")
...
def _runnetwork(epoch, train_loader, syn=False):
    ...
    for batch_idx, targets in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast():
            ...
            loss = 1.5 * loss_belief + 0.5 * loss_affinities
        ...
        with torch.no_grad():
            ...
            object_points = get_cuboid_keypoints_3d("Cookies")
            if "json_path" in targets:
                for i in range(data.shape[0]):
                    ...
                    try:
                        quat_gt, t_gt, K = load_pose_and_intrinsics(json_path, object_class="Cookies")
