...
from utils_vit import loadimages_inference, loadweights, Draw


def draw_projected_cuboids(img_path, json_data, save_path):
    ...
    for obj in json_data['objects']:
        pts = obj['projected_cuboid']
        for pt in pts:
            draw.ellipse((pt[0] - 2, pt[1] - 2, pt[0] + 2, pt[1] + 2), fill='cyan', outline='cyan')

        line_order = [[0, 1], [1, 2], [3, 2], [3, 0],  # front face
                      [4, 5], [6, 5], [6, 7], [4, 7],  # back face
...
class DopeNode(object):
    ...
    def image_callback(
        ...
        if belief_imgs is not None:
            image_idx = os.path.splitext(os.path.basename(img_name))[0]
            belief_map_name = image_idx + "_belief_maps.png"
            belief_imgs.save(os.path.join(output_path, belief_map_name))
        ...
        # draw_projected_cuboids(
        #     img_path=os.path.join(output_path, img_name_base),
        #     json_data=dict_out,
        #     save_path=os.path.join(output_path, img_name_base.replace('.png', '-validate.png'))
        # )
